...
func main() {
	...
	startPort := flag.Int("start-port", 1, "The lower bound port to begin scanning.")
	endPort := flag.Int("end-port", 1024, "The upper bound port to finish scanning.")
	workers := flag.Int("workers", 100, "The number of concurrent goroutines to launch per target.")
	timeout := flag.Int("timeout", 5, "The connection timeout in seconds.")
